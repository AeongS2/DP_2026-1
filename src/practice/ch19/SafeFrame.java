...
public class SafeFrame extends Frame implements ActionListener, Context {
    ...
    public SafeFrame(String title) {
        super(title);   // 부모 생성자 항상 제일 처음에 호출

        setBackground(Color.lightGray);
        setLayout(new BorderLayout());
        // textClock 배치 
        add(textClock, BorderLayout.NORTH);
        textClock.setEditable(false);   // 편집 불가로 설정
        // textScreen 배치 
    ...
    public void actionPerformed(ActionEvent e) {
        System.out.println(e.toString());   // e.getSource() : 어디서 발생했는지 가져옴
        if (e.getSource() == buttonUse) {		// 금고 사용 버튼
            state.doUse(this);  // 현재 상태 객체에게 위임(중요)
    ...
    // 시간 설정 : 1초에 한 번씩 시계에서 호출된다
    @Override
    public void setClock(int hour) {
        String clockstring = String.format("현재 시간은 %02d:00", hour);    // 형식을 갖춘 문자열 만들기, d = digit, 02 : 두자리로, 앞에자리 없으면 0으로 채움
